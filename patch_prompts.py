...
# Find ConceptAgent end of prompt
concept_end = "- **表达克制**：沟通时不“互吹”，用最简洁的方式说明问题。\n\"\"\""
if prompts_code.count(concept_end) > 0:
    # It might be in multiple places, let's fix replacing for specific agent definitions
    pass

import re
# Insert into Concept Agent
prompts_code = re.sub(
    r'(CONCEPT_SYSTEM_PROMPT = """.*?- \*\*表达克制\*\*：沟通时不“互吹”，用最简洁的方式说明问题。)(\n""")',
    r'\1\n' + concept_workflow + r'\2',
    prompts_code,
    flags=re.DOTALL
)

# Insert into Design Agent
prompts_code = re.sub(
    r'(DESIGN_SYSTEM_PROMPT = """.*?- \*\*表达克制\*\*：沟通时不“互吹”，用最简洁的方式说明问题。)(\n""")',
    r'\1\n' + design_workflow + r'\2',
    prompts_code,
    flags=re.DOTALL
)
